...
export class BulkEmails {
    ...
    constructor(ValidationControllerFactory, config, people, utils, is4ua, store) {
        ...
        this.filters = [
            ...
            { value: '', custom: this.filterRegion },
            { value: '', custom: this.filterCountry  },
            { value: '', custom: this.filterName }
        ];
    }

    filterRegion(filterValue, row) {
        filterValue = filterValue.toUpperCase();
        return row.institutionId && (!filterValue || row.institutionId.region == filterValue);
    }

    filterCountry(filterValue, row) {
        filterValue = filterValue.toUpperCase();
        return row.institutionId && (!filterValue || row.institutionId.copuntry == filterValue);
    }

    filterName(filterValue, row) {
        filterValue = filterValue.toUpperCase();
        return row.institutionId && (!filterValue || row.institutionId.name == filterValue);
    }
