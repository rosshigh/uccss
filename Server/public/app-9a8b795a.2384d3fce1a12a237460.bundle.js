(self["webpackChunkuccss_old_new"] = self["webpackChunkuccss_old_new"] || []).push([["app-9a8b795a"],{
...
let AppConfig = (_dec = (0,aurelia_framework__WEBPACK_IMPORTED_MODULE_0__.inject)(aurelia_http_client__WEBPACK_IMPORTED_MODULE_1__.HttpClient), _dec(_class = class AppConfig {
  constructor(http) {
    ...
    // IMG_DOWNLOAD_URL = "http://localhost/";   
    this.BASE_URL = this.HOST + "/api/";
    this.IMG_DOWNLOAD_URL = this.HOST + "/";
    this.isMobile = false;
  ...
  async getConfig(refresh) {
    if (refresh || !this.configArray) {
      return this.http.createRequest('/config').asGet().withHeader('Authorization', 'JWT ' + sessionStorage.getItem('token')).send().then(response => {
  ...
  async getSessions(refresh) {
    if (refresh || !this.SESSION_PARAMS) {
      return this.http.createRequest('/semesterConfig').asGet().withHeader('Authorization', 'JWT ' + sessionStorage.getItem('token')).send().then(response => {
        if (!response.isSuccess) {
          return response;
        } else {
...
//# sourceMappingURL=app-9a8b795a.2384d3fce1a12a237460.bundle.js.map