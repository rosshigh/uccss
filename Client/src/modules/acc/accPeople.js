...
export class ACCPeople {
    ...
    async activate() {
        ...
        let responses = await Promise.all([
            this.people.getPeopleArray('?order=lastName&filter=[and]personStatus|eq|01:apj|eq|true',true),
    ...
    async filterActive(){
		...
		if( this.activeFilterValue == "") {
			await this.people.getPeopleArray('?filter=apj|eq|true&order=lastName', true);
		} else {
			await this.people.getPeopleArray('?order=lastName&filter=[and]apj|eq|true:personStatus|eq|' + this.activeFilterValue, true); 
		}
        this.dataTable.updateArray(this.people.peopleArray);
        $('#loading').hide();
    }
    ...
    async refresh() {
        ...
        $('#loading').show();
        await this.people.getPeopleArray('?order=lastName&filter=[and]personStatus|eq|01:apj|eq|true', true),
        this.dataTable.updateArray(this.people.peopleArray);
        $('#loading').hide();
    }
    ...
    async new() {
        ...
        this.newPerson = true;
        this.oldEmail = this.people.selectedPerson.email;
        this.people.selectedPerson.apj = true;
        $("#editFirstName").focus();
        this.personSelected = true;
        this.phoneMask = "";
        this.setFirstTab();
