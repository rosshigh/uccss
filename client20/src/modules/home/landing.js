...
@inject(Auth, AppConfig, Router, Utils, Aurelia,People, is4ua, Config)
export class Landing {


    constructor(auth, appConfig, router, utils, aurelia, people, is4ua, config) {
        this.auth = auth;
        this.router = router;
    ...
    async loginSuccess() {
        this.userObj = this.utils.getUser('user');
        if (this.userObj) {
            if (this.userObj.institutionId.institutionStatus !== this.appConfig.INSTITUTIONS_ACTIVE) {
                ...
                if (this.userObj.personStatus !== this.appConfig.ACTIVE_PERSON) {
                    ...
                    this.utils.setUserRole(this.userObj.userRole);
